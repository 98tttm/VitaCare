...
export class ProductDetail implements OnInit {
  ...
  fetchHealthVideos(): void {
    ...
    this.productService.getHealthVideos({
      ...
      error: (err) => console.error('Error fetching health videos:', err)
    });
  }
  ...
  processVideos(videos: any[]) {
    this.healthVideos = (videos || []).map(v => {
      ...
      return {
        ...v,
        thumbnail: videoId ? `https://img.youtube.com/vi/${videoId}/hqdefault.jpg` : 'assets/images/placeholder.png'
      };
    });
    this.cdr.detectChanges();
  }
  ...
  calculateReviewStats() {
    ...
    if (total === 0) {
      this.reviewStats = { average: 5.0, total: 0, stars: [0, 0, 0, 0, 0] };
      return;
    }
  ...
  submitReview() {
    ...
    const reviewData = {
      ...
      fullname: '' // Backend will generate "Khách hàng vãng lai XXXX"
    };
  ...
  submitQuestion() {
    ...
    const payload = {
      ...
      full_name: '' // Backend will use default
    };
